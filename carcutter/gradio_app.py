...
def segment(image: Image.Image, text: str, threshold: float, mask_threshold: float):
    ...
    try:
        inputs = processor(images=image, text=text.strip(), return_tensors="pt").to(
            device
        )
        ...
        if n_masks == 0:
            return (
                image,
                [],
            ), f"❌ No objects found matching '{text}' (try adjusting thresholds)."
        ...
        scores_text = ", ".join(
            [f"{s:.2f}" for s in results["scores"].cpu().numpy()[:5]]
        )
