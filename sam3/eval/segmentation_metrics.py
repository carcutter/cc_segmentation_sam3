...
class SegmentationMetricsMeter:
    ...
    def __init__(
        ...
        fixed_vis_indices: bool = True,
        vis_sample_seed: int = 0,
        per_image_metrics_dir: Optional[str] = None
    ):
        """
        Args:
            ...
            fixed_vis_indices: If True, keep the same visualization samples across epochs
            vis_sample_seed: Random seed used when selecting fixed samples
        """
        self.logger = logger
        ...
        self.fixed_vis_indices = fixed_vis_indices
        self.vis_sample_seed = vis_sample_seed
        self._fixed_vis_indices = None
        self.phase = None  # Will be set by update() based on batch
        self.per_image_metrics_dir = per_image_metrics_dir
    ...
    def update(self, find_stages=None, find_metadatas=None, model=None, batch=None, key=None, phase=None, **kwargs):
        ...
        # Extract COCO image IDs from metadata
        image_ids = []
        if find_metadatas is not None:
            for meta in find_metadatas:
                if hasattr(meta, 'coco_image_ids'):
                    image_ids.extend(meta.coco_image_ids)

        # Call the original update method
        self._update_from_outputs(outputs, targets, images, image_ids=image_ids or None)
    
    def _update_from_outputs(self, outputs: Dict, targets: Dict, images: Optional[torch.Tensor] = None, image_ids: Optional[List] = None):
        """
        ...
        for i in range(batch_size):
            ...
            # Track image ID
            self.image_ids.append(image_ids[i] if image_ids and i < len(image_ids) else len(self.image_ids))
            ...
            # Image-level detection counts (requires IoU > 0.1 to count as TP/overlap)
            pred_has = pred_np.any()
            gt_has = gt_np.any()
            overlaps = iou > 0.1
            if gt_has and pred_has and overlaps:
                self.detection_tp += 1
            elif pred_has and (not gt_has or not overlaps):
                self.detection_fp += 1
    ...
    def compute(self) -> Dict[str, float]:
        ...
        precision = self.detection_tp / (self.detection_tp + self.detection_fp) if (self.detection_tp + self.detection_fp) > 0 else 0.0
        recall = self.detection_tp / (self.detection_tp + self.detection_fn) if (self.detection_tp + self.detection_fn) > 0 else 0.0
    ...
    def compute_synced(self, epoch: int = 0) -> Dict[str, float]:
        ...
        all_image_ids = all_gather(self.image_ids)
        all_det_counts = all_gather([self.detection_tp, self.detection_fp, self.detection_fn, self.detection_tn])
        ...
        all_image_ids = [item for sublist in all_image_ids for item in sublist]
        tp = sum(x[0] for x in all_det_counts)
        fp = sum(x[1] for x in all_det_counts)
        fn = sum(x[2] for x in all_det_counts)
        tn = sum(x[3] for x in all_det_counts)
        ...
        if len(all_iou_scores) == 0:
            ...
            return {
                ...
                f'{phase_prefix}_{self.metric_prefix}/detection/precision': 0.0,
                f'{phase_prefix}_{self.metric_prefix}/detection/recall': 0.0,
                f'{phase_prefix}_{self.metric_prefix}/detection/f1': 0.0,
            }
        ...
        # Write per-image metrics CSV on main process during validation
        if is_main_process() and self.per_image_metrics_dir and self.phase == 'val':
            os.makedirs(self.per_image_metrics_dir, exist_ok=True)
            path = os.path.join(self.per_image_metrics_dir, f"per_image_metrics_epoch_{epoch:04d}.csv")
            with open(path, 'w') as f:
                f.write("image_id,iou,contour_iou\n")
                for img_id, iou, c_iou in zip(all_image_ids, all_iou_scores, all_contour_iou_scores):
                    f.write(f"{img_id},{iou:.6f},{c_iou:.6f}\n")
        ...
        return {
            ...
            f'{phase_prefix}_{self.metric_prefix}/detection/precision': float(precision),
            f'{phase_prefix}_{self.metric_prefix}/detection/recall': float(recall),
            f'{phase_prefix}_{self.metric_prefix}/detection/f1': float(f1),
        }
    ...
    def _log_visualizations_to_tensorboard(self, epoch: int = 0):
        ...
        max_samples = min(self.num_vis_samples, len(self.visualization_data))

        if self.fixed_vis_indices:
            if (
                self._fixed_vis_indices is None
                or any(idx >= len(self.visualization_data) for idx in self._fixed_vis_indices)
                or len(self._fixed_vis_indices) != max_samples
            ):
                rng = random.Random(self.vis_sample_seed)
                self._fixed_vis_indices = rng.sample(range(len(self.visualization_data)), max_samples)

            samples_to_vis = [
                self.visualization_data[idx]
                for idx in self._fixed_vis_indices
                if idx < len(self.visualization_data)
            ]
        else:
            # Randomly sample num_vis_samples
            samples_to_vis = random.sample(self.visualization_data, max_samples)
